...
class RequestStatusNotOk(Exception):
...
def check_tokens():
    ...
    for variable in variables:
        if not variable:
            ...
            sys.exit(f'Отсутствует переменная среды {variables.get(variable)},'
...
def send_message(bot, message):
    """Send message."""
    logging.info(f'Начало отправки сообщений в Telegram')
    bot.send_message(
...
def main():
    ...
    while True:
        try:
            response_api = requests.get(ENDPOINT, headers=HEADERS, params={'from_date': timestamp})
            ...
            if len(homeworks) != 0:
                ...
                logger.debug('Отсутствуют новые статусы')
        except EmptyAnswerAPI:
            logger.error(EmptyAnswerAPI)
